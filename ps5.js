...
function getRhyme() {
    ...
    datamuseRequest(url, data => {
        ...
        if (data.length == 0) {
            ...
            let groupedDict = groupBy(data,"numSyllables");
            for (let item in groupedDict) {
                let wordDict = groupedDict[item];
                const syllablesHeader = document.createElement("h3")
                syllablesHeader.textContent = "Syllables: " + wordDict[0]["numSyllables"]
                wordOutput.append(syllablesHeader)
                const listContainer = document.createElement("ul")
                wordOutput.append(listContainer)

                for (let index in wordDict) {
                    // console.log(wordDict[word])
                    let word = wordDict[index]["word"]
                    let syllables = wordDict[index]["numSyllables"]
    ...
    datamuseRequest(url, data => {
        ...
        if (data.length == 0) {
            ...
            for (let item in data) {
                const {word, score, syllables} = data[item]
                const listItem = document.createElement("li");
                listItem.innerHTML = word + "</span><button class='btn btn-sm btn-outline-success save' type=button>(save)</button>";
                wordOutput.append(listItem);
